...
router.get('/', async (req, res) => {
  try {
    const items = await ShoppingList.find().populate('productId').sort({ addedAt: -1 })
    res.json(items)
  } catch (err) {
...
router.patch('/:id/quantity', async (req, res) => {
  try {
    const item = await ShoppingList.findByIdAndUpdate(req.params.id, { quantity: req.body.quantity }, { new: true })
    res.json(item)
  } catch (err) {
    res.status(400).json({ error: err.message })
  }
